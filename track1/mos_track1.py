...
def main():
    ...
    parser.add_argument('--datadir', type=str, default="/share/nas169/wago/AudioMOS/data/track1/MusicEval-phase1", required=False, help='Path of musiceval dataset')
    parser.add_argument('--expname', type=str, required=False, default='test', help='ckpt will be saved in ../ckpt/EXPNAME')
    parser.add_argument('--preference_factor', type=float, required=False, default=7.0)
    parser.add_argument('--epsilon', type=float, required=False, default=0.1)
    parser.add_argument('--margin_scale', type=float, required=False, default=0.2)
    args = parser.parse_args()

    DATA_DIR = args.datadir
    UPSTREAM_MODEL = 'CLAP-music'
    EXP_NAME = args.expname
    CKPT_DIR = '../ckpt/' + EXP_NAME # checkpoint will be saved here
    ...
    trainloader = DataLoader(trainset, batch_size=16, shuffle=True, num_workers=8, collate_fn=trainset.collate_fn)
    ...
    for epoch in range(1,1001):
        ...
        for i, data in enumerate(tqdm(trainloader, desc="Training Progress", ncols=100), 0):
            ...
            rank_loss1 = preference_aware_pamr_loss(output1, labels1)
            loss1 = criterion(output1, labels1) + rank_loss1
            rank_loss2 = preference_aware_pamr_loss(output2, labels2)
            loss2 = criterion(output2, labels2) + rank_loss2
